...
1. [nix-installer](https://github.com/NixOS/nix-installer) を使い、Nixをインストールする[^2]
   ```sh
...
4. [zmk-workspace](https://github.com/kot149/zmk-workspace)をcloneする
   :::message alert
...
4. uf2ファイルを書き込む
   ...
   just flash roBa_R -r
   ```
5. Keymap Drawerでキーマップ画像を生成する
   ```sh
   just draw
   ```
   生成された画像は、`<zmk-configディレクトリ>/keymap-drawer/<名前>.svg`、`<名前>.yml`に保存されます。
