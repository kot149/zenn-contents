...
:::details 分割型キーボードのperipheral側に入力デバイスがある場合
入力デバイスがperipheral側にある場合、以下のような流れで入力イベントが処理される。

> (peripheral上の)入力デバイス → input-split → central → input-listener → HIDレポート

この際、input processorはinput-splitとinput-listenerの両方で設定可能であり、input-splitで設定した場合はperipheral側で、input-listenerで設定した場合はcentral側で処理される。
...
一部の機能(例えばキー入力状態の監視など)はcentral側でしか実装できないという制約があるため、input-listenerで設定する必要がある。そのため、基本的にinput-listenerで設定した方が無難。

一方で、[入力イベントのレポート頻度を制限するinput processor](#入力イベントのレポート頻度を制限する) のように、centralへ送る前に行いたい処理も存在する。また、処理の負荷分散の観点から、input-split側で処理できるものはそちらで処理する方がいいという考えもあり得るが、あえて偏らせた方が充電の手間が減るという話もあり、そこは人によるだろう。
:::
