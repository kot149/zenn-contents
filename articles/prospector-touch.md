...
:::message
写真ではbeekeebのコネクタボードのJST GHコネクタの足からマイコンのピンへ配線していますが、このやり方は推奨しません。コネクタの足の部分はかなり狭く、はんだ付けが難しいです。
beekeebのコネクタボードを使用する場合は、コネクタの前でタッチ用のケーブル4本を切断して、コネクタを通さずにマイコンへ直接配線する方が簡単です。
:::

![](https://storage.googleapis.com/zenn-user-upload/d2be3daeb869-20251109.png)
